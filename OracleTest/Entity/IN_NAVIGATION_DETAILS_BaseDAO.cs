...
{
	/// 
	/// 对于IN_NAVIGATION_DETAILSBaseDAO的类的实现（基础控制部分）。
	/// 对于扩展的DAO的实现可以从这个类继承。
	/// 
	public class IN_NAVIGATION_DETAILSBaseDAO : MarshalByRefObject , IDisposable
	{
		#region variable
	  ...
	  //protected bool m_isNeedClose = true;
		...
		/// 结构化一个BaseDAO。 
		/// 
		public IN_NAVIGATION_DETAILSBaseDAO()
		{
		...
		//public IN_NAVIGATION_DETAILSBaseDAO(IDataAccess cdb)
		public IN_NAVIGATION_DETAILSBaseDAO(IDataBaseOperate cdb)
		{
			db = cdb;
		...
		/// 
		/// 更新IN_NAVIGATION_DETAILS实体到数据库。
		/// 
		/// 将要更新的实体
		public int UpdateEntity(DataSetIN_NAVIGATION_DETAILS entity)
		{
			CheckData(entity);
			int iRtn=0;
			bool isExistTrans = db.IsInTransaction();
			...
			try
			{
				...
				foreach(DataRow row in entity.Tables["YWAPP_INHOSPITAL_NAVIGATION_DETAIL"].Rows)
        {
          int j =	db.exeSql(new SS_IN_NAVIGATION_DETAILS_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL(), row);
          iRtn = iRtn+j;
          }
    ...
    throw e;
		...
		/// 
		/// 从数据库中删除IN_NAVIGATION_DETAILS实体。
		/// 
		/// 将要删除的实体
		public int DeleteEntity(DataSetIN_NAVIGATION_DETAILS entity)
		{
			CheckData(entity);
      int iRtn=0;
			bool isExistTrans = db.IsInTransaction();
			...
			try
			{
				...
				foreach(DataRow row in entity.Tables["YWAPP_INHOSPITAL_NAVIGATION_DETAIL"].Rows)
				{
					int j = db.exeSql(new SS_IN_NAVIGATION_DETAILS_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL(), row);
          iRtn = iRtn+j;
         }
    ...
    public int CommitChanges(DataSetIN_NAVIGATION_DETAILS entity)
      ...
      int iRtn=0;
		...
		#region Data_IN_NAVIGATION_DETAILS_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL
		/// 
		/// 对于DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL的类的实现（数据库操作）。
		/// 
		public int Data_IN_NAVIGATION_DETAILS_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL ( System.String ID)
		{
		    int iRtn;
			bool isExistTrans = db.IsInTransaction();
			if(!isExistTrans)
			...
			SS_IN_NAVIGATION_DETAILS_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL ss = new SS_IN_NAVIGATION_DETAILS_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL();
			...
			return iRtn;
		} // Data_DeleteYWAPP_INHOSPITAL_NAVIGATION_DETAIL
		#endregion	
		
		#region Data_IN_NAVIGATION_DETAILS_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL
		/// 
		/// 对于UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL的类的实现（数据库操作）。
		/// 
		public int Data_IN_NAVIGATION_DETAILS_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL ( System.String INID , System.String ID , System.String FLOORNUM , System.String DEPARTCODE , System.String DEPARTNAME , System.Byte[] FLOORNUMMAP , System.String OLDID)
		{
		    int iRtn;
			bool isExistTrans = db.IsInTransaction();
			if(!isExistTrans)
			...
			SS_IN_NAVIGATION_DETAILS_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL ss = new SS_IN_NAVIGATION_DETAILS_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL();
				
			ss.INID = INID;
				
			ss.ID = ID;
				
			ss.FLOORNUM = FLOORNUM;
				
			ss.DEPARTCODE = DEPARTCODE;
				
			ss.DEPARTNAME = DEPARTNAME;
				
			ss.FLOORNUMMAP = FLOORNUMMAP;
			...
			return iRtn;
		} // Data_UpdateYWAPP_INHOSPITAL_NAVIGATION_DETAIL
		#endregion	
